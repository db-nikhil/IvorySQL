 ...
 *
 * This code need not be called if the plisql function contains no exception
...
mark_stmt(PLiSQL_stmt * stmt, Bitmapset *local_dnos)
{
	if (stmt == NULL)
		return;
	if (stmt->cmd_type == PLISQL_STMT_BLOCK)
	{
		PLiSQL_stmt_block *block = (PLiSQL_stmt_block *) stmt;
		...
		if (block->exceptions)
		{
			 ...
			 */
			plisql_statement_tree_walker(stmt, mark_stmt, mark_expr, NULL);
		}
		else
		{
			/*
		...
		plisql_statement_tree_walker(stmt, mark_stmt, mark_expr, local_dnos);
}
...
plisql_mark_local_assignment_targets(PLiSQL_function * func)
{
	...
	mark_stmt((PLiSQL_stmt *) func->action, local_dnos);
	bms_free(local_dnos);
}
